...
test('Check readOnly comment flat is moving text bruck inside', async () => {
    ...
    const comment = <TextFieldBrick>await SurveyHelper.createCommentFlat({ xLeft: 0, yTop: 0 }, question, controller, { isMultiline: true, value: question.value, rows: question.rows } as any);
