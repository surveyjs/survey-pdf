...
export class SurveyHelper {
    ...
    private static htmlMargins(controller: DocController) {
        return {
    ...
    public static async createHTMLFlat(point: IPoint, question: Question,
        ...
        return new Promise((resolve) => {
            controller.helperDoc.fromHTML(html, point.xLeft, margins.top, {
                ...
                let y: number = (controller.helperDoc.getNumberOfPages() - 1) *
                    (controller.paperHeight - controller.margins.bot -
                        controller.margins.top) + result.y - margins.top;
                for (let i: number = 0; i < controller.helperDoc.getNumberOfPages() - 1; i++) {
                    controller.helperDoc.deletePage(1);
                }
                let rect: IRect = SurveyHelper.createRect(point, margins.width, y);
