...
export class FlatHTML extends FlatQuestion<QuestionHtmlModel, IQuestionHtmlStyle> {
    ...
    public async generateFlatsContent(point: IPoint): Promise<IPdfBrick[]> {
        ...
        const html: string = SurveyHelper.createHtmlContainerBlock(SurveyHelper.getLocString(this.question.locHtml), this.controller, this.style.text);
        if (renderAs === 'image') {
        ...
        return [SurveyHelper.splitHtmlRect(this.controller, await SurveyHelper.createHTMLFlat(
            point, this.controller, this.correctHtml(html), this.style.text))];
    }
}
