...
export class FlatMatrix extends FlatQuestion {
    ...
    protected generateFlatsHeader(point: IPoint): IPdfBrick[] {
        ...
        let curPoint: IPoint = Object.assign({}, point);
        this.question.visibleColumns.forEach((column: any, index: number) => {
            ...
            curPoint.xLeft = this.controller.margins.marginLeft;
            headers.push(SurveyHelper.createBoldTextFlat(curPoint,
    ...
    protected generateFlatsRows(point: IPoint, isVertical: boolean): IPdfBrick[] {
        ...
        this.question.visibleRows.forEach((row: any, index: number) => {
            let key = "row" + index;
            let flatsRow = new FlatMatrixRow(this.question, this.controller, row, key, index == 0, isVertical).generateFlatsContent(curPoint);
            curPoint = SurveyHelper.createPoint(SurveyHelper.mergeRects(...flatsRow));
            cells.push(...flatsRow);
        });
        return cells;
    }
    public generateFlatsContent(point: IPoint): IPdfBrick[] {
        ...
        if (!isVertical && this.question.showHeader) {
            let headers: IPdfBrick[] = this.generateFlatsHeader(curPoint);
            curPoint = SurveyHelper.createPoint(SurveyHelper.mergeRects(...headers));
            cells.push(...headers);
        }
        cells.push(...this.generateFlatsRows(curPoint, isVertical));
        return cells;
...
export class FlatMatrixRow extends FlatQuestion {
    ...
    generateFlatsContent(point: IPoint): IPdfBrick[] {
        ...
        let cells: IPdfBrick[] = [];
        if (this.isVertical) {
            cells.push(...this.generateFlatsVerticallyCells(curPoint))
        }
        else {
            cells.push(...this.generateFlatsHorizontallyCells(curPoint));
        }
        return cells;
    ...
    generateFlatsHorizontallyCells(point: IPoint): IPdfBrick[] {
        ...
        if (this.question.hasRows) {
            ...
            curPoint.xLeft = this.controller.margins.marginLeft
            cells.push(SurveyHelper.createTextFlat(curPoint, this.question, this.controller, SurveyHelper.getLocString(this.row.locText), TextBrick));
            this.controller.margins.marginLeft = oldLeftMargin;
            this.controller.margins.marginRight = oldRightMargin;
        }
        this.question.visibleColumns.forEach((column, index) => {
            ...
            if (this.question.hasCellText) {
                cells.push(SurveyHelper.createTextFlat(curPoint, this.question, this.controller,
                ...
                let itemRect: IRect = SurveyHelper.createRect(curPoint, height, height);
        ...
        return [new CompositeBrick(...cells)];
    }
    ...
    generateFlatsVerticallyCells(point: IPoint): IPdfBrick[] {
        ...
        let cells: IPdfBrick[] = [];
        if (this.question.hasRows) {
            let rowTextFlat = SurveyHelper.createTextFlat(curPoint, this.question,
                this.controller, SurveyHelper.getLocString(this.row.locText), TextBrick);
            curPoint.yTop = rowTextFlat.yBot;
            cells.push(rowTextFlat);
        }
        this.question.visibleColumns.forEach((column, index) => {
            let checked = this.row.value == column.value;
            if (this.question.hasCellText) {
                let cellTextFlat = SurveyHelper.createTextFlat(curPoint, this.question, this.controller,
                    SurveyHelper.getLocString(this.question.getCellDisplayLocText(this.row.name, column)), TextBrick);
                curPoint.yTop = cellTextFlat.yBot;
                ...
                let radioItem: IPdfBrick = new RadioItemBrick(this.question, this.controller, itemRect, column.value,
                    checked, radioGroupWrap, index == 0);
                curPoint = SurveyHelper.createPoint(radioItem, false, true);
                let radioText: IPdfBrick = SurveyHelper.createTextFlat(curPoint, this.question, this.controller, SurveyHelper.getLocString(column.locText), TextBrick);
                curPoint = SurveyHelper.createPoint(SurveyHelper.mergeRects(radioItem, radioText));
